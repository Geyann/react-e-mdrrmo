...
/* =============================================================
 *  NETWORK SAFETY
 *  A hung request must never leave the UI stuck on a spinner.
 * ============================================================= */
...
/** Runs a Supabase builder, returns `fallback` on any failure. */
...
/* =============================================================
 *  STORAGE
 *  Three independent keys per user:
 *    mdrrmo_notif_list_<key>   notification records
 *    mdrrmo_notif_read_<key>   { notificationId → timestamp }
 *    mdrrmo_notif_cache_<key>  { table::id → last status }
 * ============================================================= */
...
/** Deterministic id — no timestamps, so read markers survive rebuilds. */
...
/* =============================================================
 *  MATCHING
 *  Tier 1 — any column equals one of our ids
 *  Tier 2 — a designated column equals our email
 *  Tier 3 — a designated column equals our mobile digits
 * ============================================================= */

const matchRow = (src, row, w) => {
  if (!w.idSet || w.idSet.size === 0) return { ok: false, via: null };
...
export default function Notification() {
  ...
  /* ── persistence (never inside a state updater) ─────────────── */
  ...
  const scanSource = useCallback(
    async (src, w) => {
      ...
      if (rows === null) {
        setBlocked((p) =>
          p.includes(src.table) ? p : [...p, src.table]
        );
        setDiag((d) => ({
          ...d,
          [src.table]: { error: "unreadable or timed out", mine: 0 },
        }));
        return;
      }
      ...
      const wantsQueue =
        src.queueRoles.includes(w.role) ||
      ...
      for (const row of rows) {
        ...
        const { ok, via } = matchRow(src, row, w);
        const prev = cacheRef.current[key];

        cacheRef.current[key] = status;
        if (ok) mine.push({ row, status, via });
        ...
        if (w.isHandler && isQueue) {
          if (prev === undefined) {
            ...
            if (ok) onStatus(src, row, "self", status, prev);
      ...
      /* Backfill uses deterministic ids, so a read marker from an
         earlier session still applies and it never re-alerts. */
  ...
  /* ── boot ──────────────────────────────────────────────────────
     StrictMode-safe: the work happens once and is stored as a
     promise. Every mount attaches its own .then to set state, so a
     cleanup/remount cycle can never strand `booting` on true.     */
  ...
  /* ── realtime (Supabase Auth sessions only) ──────────────────── */
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public" }, (pl) => {
        ...
        if (pl.eventType === "INSERT" && seededRef.current) {
          if (!w.isHandler && ok) onCreated(src, row, "self");
  ...
  /* ── read state (separate persisted store) ──────────────────── */
  ...
  const filters = [
    ...
    ...(who.isHandler
      ...
      : []),
    { id: "self", label: "Mine", count: selfUnread },
  ];
            ...
            {open && pos && (
              ...
              >
                    ...
                    {notifications.length > 0 && (
                      <>
                        ...
                        <button
                          ...
                          className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-red-600 transition"
                        >
                    ...
                    list.map((n) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() => openItem(n)}
                          className={`flex w-full items-start gap-3 border-b border-slate-50 dark:border-slate-700/60 px-4 py-3 text-left transition hover:bg-slate-50 dark:hover:bg-slate-700/40 ${
                            wasRead
                ...
                <div className="border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                  ...
                  {DEBUG && diagRows.length > 0 && (
                    <details className="px-4 py-2">
                      <summary className="text-[10px] text-slate-400 dark:text-slate-500 cursor-pointer select-none flex items-center gap-1">
                        ...
                        Match diagnostics ({diagRows.length} tables) ·{" "}
                        {Object.keys(readRef.current).length} read markers
                      </summary>
                  ...
                  <p className="px-4 py-2 text-[10px] text-slate-400 dark:text-slate-500 text-center">
                    {who.isHandler ? "Admin/Staff" : "Resident"} · watching{" "}
                    {sourcesFor(who).length} tables · every{" "}
              ...
              (() => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`border-b px-6 py-4 ${TONE_HEADER[selected.tone]}`}
                      >
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex flex-wrap justify-end gap-3">
                          <button
                            ...
                            className="rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2 text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 transition"
                          >
                            Mark as unread
                          </button>
                          <button
                        ...
                        <p className="mt-4 text-[10px] text-slate-400 dark:text-slate-500 text-center">
                          {wasRead ? "Saved as read on this device." : ""}
                        </p>
                      </div>
                    </div>
                  </div>
